...
async function streamingStatus() {
  ...
  const [control, webui] = await Promise.all([portOpen(47989), portOpen(47990)]);
  const available = control || webui;

  return {
    available,
    mode: available ? 'moonlight' : null,
    host,
    // Moonlight registers this scheme when installed.
    moonlightUrl: available && host ? `moonlight://${host}` : null,
    sunshineWebUi: webui && host ? `https://${host}:47990` : null,
    note: available
      ? 'Sunshine detected. The Stream button opens the Moonlight client pointed at this PC.'
      : 'No Sunshine host detected on this PC. Install Sunshine (LizardByte) plus the Moonlight client to stream; launching games remotely works either way.',
    downloads: available
      ? null
...
async function connect() {
  ...
  socket.on('message', (raw) => {
    ...
    if (message.type === 'registered') {
      ...
      console.log(`  ✔ Streaming: ${streaming.available ? 'Sunshine detected' : 'not available'}\n`);
